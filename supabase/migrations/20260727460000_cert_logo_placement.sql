-- quiz_settings.cert_logo_url has existed since the original quiz_settings
-- migration but was never wired to anything — this adds the missing
-- placement choice and threads both through the certificate-issuance RPCs
-- and the canvas renderer.

alter table quiz_settings add column if not exists cert_logo_position text not null default 'top_center';
alter table quiz_settings add constraint quiz_settings_cert_logo_position_check
  check (cert_logo_position in ('top_center', 'top_left', 'top_right', 'watermark'));
