-- Two things:
-- 1. A candidate photo on the certificate — admin-attached (not
--    self-upload; there's no participant-facing upload flow), so it
--    lives on the issued quiz_certificates row itself, editable by an
--    admin after the fact from the Results screen. photo_enabled on
--    the DESIGN (quiz_cert_templates) controls whether the layout
--    reserves space for a photo at all; the actual image is per
--    certificate.
-- 2. Two new "premium" templates with a more dimensional look
--    (embossed double-border + metallic gradient text, and a
--    procedurally-drawn wax-seal medallion) — still pure canvas
--    drawing, no image assets, consistent with the existing 5.

alter table quiz_cert_templates drop constraint if exists quiz_cert_templates_template_check;
alter table quiz_cert_templates add constraint quiz_cert_templates_template_check
  check (template in ('classic_gold', 'royal_blue', 'modern_purple', 'minimal_white', 'dark_elegant', 'premium_embossed', 'royal_seal'));

alter table quiz_cert_templates add column if not exists photo_enabled boolean not null default false;

alter table quiz_certificates add column if not exists candidate_photo_url text;

create policy quiz_certificates_admin_update on quiz_certificates for update
...
-- Both issuance RPCs now also return photo_enabled (from the active
-- design) and candidate_photo_url (from the certificate row itself,
-- so a photo attached after issuance shows up next time this is called).
