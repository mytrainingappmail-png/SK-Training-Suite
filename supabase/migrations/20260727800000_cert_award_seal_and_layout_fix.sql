-- Adds an optional award badge (gold medal or wax seal) above the
-- signature line, independent of which base template/decorations a
-- design already has — the "gold ribbon/medal" feature requested
-- alongside the logo/company-name overlap and overflow fixes (those
-- are renderer-only changes, no schema involved).

alter table quiz_cert_templates add column if not exists award_seal text not null default 'none'
  check (award_seal in ('none', 'medal', 'seal'));
...
returns table (
  ...
  signature_mode text, signature_align text, photo_enabled boolean, cert_photo_frame text, cert_award_seal text, candidate_photo_url text
)
language plpgsql
security definer
...
begin
  ...
  select
    ...
    coalesce(v_cert.photo_enabled, false), coalesce(v_cert.photo_frame, 'circle'), coalesce(v_cert.award_seal, 'none'), qc.candidate_photo_url
