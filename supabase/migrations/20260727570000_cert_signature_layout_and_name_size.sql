-- Signature layout is currently hardcoded to always draw two fixed slots
-- (left/right of center) whenever either signatory name is set, so a
-- single signatory always renders off-center. This adds an explicit,
-- white-label "how many signatures, and where" choice, plus an
-- independent size control for the signatory NAME text (mirroring the
-- existing image-scale columns), so both the signature image and the
-- printed name can be made bigger/smaller on the certificate.

alter table quiz_settings add column if not exists cert_signature_mode text not null default 'both';
alter table quiz_settings add constraint quiz_settings_cert_signature_mode_check
  check (cert_signature_mode in ('both', 'single'));

alter table quiz_settings add column if not exists cert_signature_align text not null default 'center';
alter table quiz_settings add constraint quiz_settings_cert_signature_align_check
  check (cert_signature_align in ('left', 'center', 'right'));

alter table quiz_settings add column if not exists cert_signatory1_name_scale int not null default 100;
alter table quiz_settings add constraint quiz_settings_cert_signatory1_name_scale_range
  check (cert_signatory1_name_scale between 50 and 150);

alter table quiz_settings add column if not exists cert_signatory2_name_scale int not null default 100;
alter table quiz_settings add constraint quiz_settings_cert_signatory2_name_scale_range
  check (cert_signatory2_name_scale between 50 and 150);

-- Both certificate-issuance RPCs pull display fields live from quiz_settings
-- at query time — adding an output column changes RETURNS TABLE, so both
-- must be dropped first (same pattern as the two migrations before this).
...
begin
  ...
  select
    ...
    v_settings.cert_logo_url, coalesce(v_settings.cert_logo_position, 'top_center'),
    ...
    coalesce(v_settings.cert_signatory1_scale, 100), coalesce(v_settings.cert_signatory1_name_scale, 100),
    v_settings.cert_signatory2_name, v_settings.cert_signatory2_title, v_settings.cert_signatory2_image_url,
    coalesce(v_settings.cert_signatory2_scale, 100), coalesce(v_settings.cert_signatory2_name_scale, 100),
    coalesce(v_settings.cert_signature_mode, 'both'), coalesce(v_settings.cert_signature_align, 'center')
