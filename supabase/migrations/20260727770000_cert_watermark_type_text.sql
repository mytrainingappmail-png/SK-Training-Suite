-- Refines the watermark toggle just added a migration ago into a proper
-- 3-way choice (None / Logo / Custom Text), the same "Picture watermark
-- or Text watermark" idea Word's own watermark dialog uses — a plain
-- on/off checkbox didn't leave room for a diagonal "CONFIDENTIAL" /
-- "SAMPLE" style text watermark, only ever the logo image.

alter table quiz_cert_templates add column if not exists watermark_type text not null default 'none' check (watermark_type in ('none', 'logo', 'text'));
alter table quiz_cert_templates add column if not exists watermark_text text;

update quiz_cert_templates set watermark_type = 'logo' where logo_watermark_enabled = true;

alter table quiz_cert_templates drop column if exists logo_watermark_enabled;
...
returns table (
  ...
  company_name text, company_name_align text, cert_logo_url text, cert_logo_position text, cert_logo_scale int, cert_watermark_type text, cert_watermark_text text, cert_title text, achievement_line text,
...
begin
  ...
  select
    ...
    coalesce(v_cert.watermark_type, 'none'), v_cert.watermark_text,
