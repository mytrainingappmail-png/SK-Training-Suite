-- Live Quiz certificates: the "Logo as Watermark" option always faded out
-- the SAME image used for the small positioned logo mark — no way to use
-- a different image (e.g. a big wordmark) just for the watermark without
-- also changing the corner logo. This adds an optional, separate
-- watermark image; when set it's used instead of the logo for the
-- watermark, but the logo itself (and its position/size) is untouched.

alter table quiz_cert_templates add column if not exists watermark_image_url text;

-- issue_my_certificate / issue_certificate_for_participant read every
-- rendering field live from quiz_cert_templates at issuance time (not
-- snapshotted onto quiz_certificates) — same pattern as watermark_type/
-- watermark_text already use, so only these two functions need updating.
-- CREATE OR REPLACE can't add a return column in place, so both are
-- dropped first.
