-- Lets the design decide what shape the candidate photo slot is cropped
-- into (circle, square, rounded square, hexagon, oval, or a polaroid-style
-- card) instead of the fixed circle it always was — purely cosmetic, and
-- optional like everything else about the photo feature.

alter table quiz_cert_templates add column if not exists photo_frame text not null default 'circle'
...
returns table (
  ...
  signature_mode text, signature_align text, photo_enabled boolean, cert_photo_frame text, candidate_photo_url text
)
language plpgsql
security definer
...
begin
  ...
  select
    ...
    coalesce(v_cert.photo_enabled, false), coalesce(v_cert.photo_frame, 'circle'), qc.candidate_photo_url
