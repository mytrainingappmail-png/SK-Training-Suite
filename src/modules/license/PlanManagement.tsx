...
function Toggle({ on, onChange, disabled }: { on: boolean; onChange: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onChange} disabled={disabled} className={`relative inline-flex h-5 w-9 flex-shrink-0 rounded-full transition disabled:opacity-50 ${on ? 'bg-indigo-600' : 'bg-slate-300'}`}>
...
function PlanManagement() {
  ...
  const [planModuleStates, setPlanModuleStates] = useState<CompanyModuleState[]>([]);
  ...
  function openEdit(plan: SubscriptionPlan) {
    ...
    getPlanModuleStates(plan.id)
      .then(setPlanModuleStates)
      .catch(() => setPlanModuleStates([]))
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        ...
        // Stay open and switch into edit mode for the newly created plan —
        // module selection needs a real plan_id, which only exists after this.
        const created = await saveNewPlan(form);
        fetchAll();
        openEdit(created);
      }
  ...
  async function handleTogglePlanModule(moduleKey: string, next: boolean) {
    if (!editingId) return;
    setSavingModuleKey(moduleKey);
    setPlanModuleStates((prev) => prev.map((s) => (s.key === moduleKey ? { ...s, enabled: next, hasOverride: true } : s)));
    try {
      await setPlanModule(editingId, moduleKey, next);
    } catch (err) {
      showToast(err instanceof Error ? err.message : 'Failed to save module.');
      getPlanModuleStates(editingId).then(setPlanModuleStates);
    } finally {
          ...
          <div className="relative z-10 max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <div className="border-t pt-3">
                <p className="mb-1 text-xs font-semibold text-slate-500">Modules Included</p>
                {!editingId ? (
                  <p className="text-xs text-slate-400">Save the plan first, then choose which sections/add-ons it includes.</p>
                ) : modulesLoading ? (
                  <p className="text-xs text-slate-400">Loading modules…</p>
                ) : (
                  <div className="max-h-56 space-y-1 overflow-y-auto rounded-lg bg-slate-50 p-2">
                    {planModuleStates.map((m) => (
                      <div key={m.key} className="flex items-center gap-2 rounded-lg px-2 py-1.5">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-xs font-semibold text-slate-700">
                            {m.label}
                            {m.is_addon && <span className="ml-1.5 rounded-full bg-amber-50 px-1.5 py-0.5 text-[9px] font-semibold text-amber-700">Add-on</span>}
                          </p>
                        </div>
                        <Toggle on={m.enabled} onChange={() => handleTogglePlanModule(m.key, !m.enabled)} disabled={savingModuleKey === m.key} />
                      </div>
                    ))}
                  </div>
                )}
