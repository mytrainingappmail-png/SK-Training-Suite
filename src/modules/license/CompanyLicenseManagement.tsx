...
import { loadCompanies, saveCompany, removeCompany, renameCompanyProfile } from '../../services/company/companyService';
import { sendEmail } from '../../repositories/email/emailRepository';
import { defaultCompanyLicenseForm, planAmountForCycle } from '../../types/license';
...
function CompanyLicenseManagement() {
  ...
  function openRename(c: Company) {
    setRenameTarget(c);
    setRenameName(c.company_name);
    setRenameCode(c.company_code);
    setRenameError('');
  }
  ...
  async function handleRenameSave() {
    ...
    setRenameError('');
    try {
      await renameCompanyProfile(renameTarget.id, renameName, renameCode);
      setRenameTarget(null);
      fetchAll();
      showToast('Company name/code updated. Employees signing in from now on need the new code.');
    } catch (err) {
      setRenameError(err instanceof Error ? err.message : 'Failed to save.');
    } finally {
      setRenaming(false);
    }
  }
  ...
  async function handleSendPaymentLink(lic: CompanyLicense) {
    ...
    const amount = plan ? planAmountForCycle(plan, lic.billing_cycle) : 0;
    ...
    <div className="space-y-6">
            ...
            {companies.map((c) => (
              <div key={c.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 px-3 py-2.5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-slate-100 px-2 py-0.5 font-mono text-[10px] font-semibold text-slate-600">{c.company_code}</span>
                ...
                {c.active ? (
                  <div className="flex gap-2">
                    <SecondaryButton onClick={() => openRename(c)}>Rename / Change Code</SecondaryButton>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.billing_cycle} onChange={(e) => setForm((f) => ({ ...f, billing_cycle: e.target.value as 'monthly' | 'six_month' | 'yearly' }))} className={INPUT_CLS}>
      ...
      {renameTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="mb-1 text-lg font-bold text-slate-900">Rename {renameTarget.company_name}</h3>
            <p className="mb-4 text-sm text-slate-500">The code is what employees type as "Company Code" at login. It lives in one place, so changing it here updates it everywhere at once — nothing else needs to change.</p>
            <div className="space-y-3">
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Company Name</label>
                <input value={renameName} onChange={(e) => setRenameName(e.target.value)} className="w-full rounded-lg bg-slate-50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400/40" />
              </div>
              <div>
                <label className="mb-1 block text-xs font-semibold text-slate-500">Company Code</label>
                <input value={renameCode} onChange={(e) => setRenameCode(e.target.value.toUpperCase())} className="w-full rounded-lg bg-slate-50 px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-400/40" />
              </div>
              {renameCode.trim().toUpperCase() !== renameTarget.company_code && (
                <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
                  ⚠ Every employee (and any quiz/calling-app admin) will need to use "{renameCode.trim().toUpperCase()}" instead of "{renameTarget.company_code}" the next time they log in. Tell them before you save.
                </p>
              )}
              {renameError && <p className="text-xs text-red-600">{renameError}</p>}
            </div>
            <div className="mt-5 flex justify-end gap-2">
              <SecondaryButton onClick={() => setRenameTarget(null)} disabled={renaming}>Cancel</SecondaryButton>
              <PrimaryButton onClick={handleRenameSave} disabled={renaming || !renameName.trim() || !renameCode.trim()}>
                {renaming ? <IconSpinner className="h-3.5 w-3.5" /> : null} Save
              </PrimaryButton>
            </div>
          </div>
        </div>
