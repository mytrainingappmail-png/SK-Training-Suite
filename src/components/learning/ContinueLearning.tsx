...
// Rotating gradient palette so cards read as colorful even without a
// course thumbnail image.
const CONTINUE_GRADIENTS = [
      ...
      <div className={`relative h-32 w-full bg-gradient-to-br ${gradient}`}>
        {item.courseThumbnail ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-white/70">
            <svg className="h-9 w-9" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
            </svg>
          </div>
        )}
        <span className="absolute right-3 top-3 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-bold text-slate-700 shadow-sm">
      ...
      {!loading && !error && filtered.length > 0 && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item, index) => (
            <ContinueCard key={item.enrollmentId} item={item} index={index} onContinue={handleContinue} />
          ))}
        </div>
      )}
