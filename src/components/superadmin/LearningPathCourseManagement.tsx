...
import { defaultLearningPathCourseForm } from "../../types/learningPathCourse";
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="mb-1 text-base font-semibold text-slate-800">
        {search ? "No courses found" : "No courses in any learning path yet"}
      </h3>
      <p className="mb-6 text-sm text-slate-500">
        {search ? `No results for "${search}".` : "Add a course to a learning path to get started."}
      </p>
      {!search && (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="dd-title">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <h3 id="dd-title" className="mb-1 text-lg font-semibold text-slate-800">Remove Course from Path</h3>
        <p className="mb-6 text-sm text-slate-500">
          Are you sure you want to remove <span className="font-semibold text-slate-700">{name}</span> from this learning path? This cannot be undone.
        </p>
...
function LearningPathCourseModal({
  editing,
  learningPaths,
  courses,
  items,
  ...
  editing: LearningPathCourse | null;
  learningPaths: LearningPath[];
  courses: Course[];
  items: LearningPathCourse[];
  saving: boolean;
  onSave: (data: LearningPathCourseForm) => void;
  onClose: () => void;
}) {
  const isEdit = editing !== null;

  const [form, setForm] = useState<LearningPathCourseForm>(() =>
    isEdit
      ? {
          ...
          unlock_previous:    editing.unlock_previous,
          estimated_duration: editing.estimated_duration,
          active:             editing.active,
        }
      : { ...defaultLearningPathCourseForm }
  );
  ...
  function field<K extends keyof LearningPathCourseForm>(key: K, val: LearningPathCourseForm[K]) {
    setForm((p) => ({ ...p, [key]: val }));
  ...
  function validate(): boolean {
    ...
    if (form.learning_path_id && form.course_id) {
      const dup = items.some(
        (item) =>
          item.learning_path_id === form.learning_path_id &&
          item.course_id        === form.course_id &&
          (!isEdit || item.id !== editing.id)
      );
      if (dup) e.course_id = "This course is already in the selected learning path.";
    }

    if (form.sequence_no < 1) {
      e.sequence_no = "Sequence Number must be greater than zero.";
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="lpc-form-title"
    >
      ...
      <div className="relative z-10 w-full max-w-xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 id="lpc-form-title" className="text-lg font-semibold text-slate-800">
              {isEdit ? "Edit Course in Path" : "Add Course to Path"}
            </h2>
            <p className="text-sm text-slate-500">
              {isEdit ? "Update course assignment details." : "Assign a course to a learning path."}
            </p>
          </div>
        ...
        <form onSubmit={onSubmit} noValidate>
          <div className="space-y-5 p-6">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <FL label="Sequence Number" required error={errs.sequence_no}>
                <input type="number" min={1} value={form.sequence_no}
                  onChange={(e) =>
                    field("sequence_no", Math.max(1, parseInt(e.target.value, 10) || 1))
                  }
            ...
            <div className="flex flex-wrap gap-y-4 gap-x-8 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <ToggleRow
                label="Mandatory"
                sub="Learner must complete this course"
                on={form.mandatory}
                onChange={() => field("mandatory", !form.mandatory)}
                disabled={saving}
              />
              <ToggleRow
                label="Unlock Previous"
                sub="Requires previous course to be completed first"
                on={form.unlock_previous}
                onChange={() => field("unlock_previous", !form.unlock_previous)}
                disabled={saving}
              />
              <ToggleRow
                label="Active"
                sub="Course is available within this path"
                on={form.active}
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? "Saving…" : isEdit ? "Update" : "Add to Path"}
            </button>
          </div>
        </form>
...
type ModalKind =
  | { type: "add" }
  | { type: "edit"; item: LearningPathCourse }
  | { type: "delete"; item: LearningPathCourse }
  | null;

export default function LearningPathCourseManagement() {

  const [items,         setItems]         = useState<LearningPathCourse[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    if (kw) {
      rows = rows.filter((r) => {
        const cName = findName(courses, r.course_id, "course_name");
    ...
    return rows;
  }, [items, courses, learningPaths, search, pathFilter]);
  ...
  useEffect(() => { setPage(1); }, [search, pathFilter]);
  ...
  const load = useCallback(async () => {
    ...
    setBanner("");
    try {
      const [itemData, lpData, courseData] = await Promise.all([
        loadLearningPathCourses(),
        loadLearningPaths(),
        loadCourses(),
      ]);
  ...
  const handleSave = useCallback(
    async (data: LearningPathCourseForm) => {
      setSaving(true);
      try {
        if (modal?.type === "edit") {
          await saveLearningPathCourse(modal.item.id, data);
        } else {
          await createLearningPathCourse(data);
        }
        await load();
        setBanner("");
        closeModal();
      } catch (err) {
        setBanner(err instanceof Error ? err.message : "Unable to save.");
      } finally {
        setSaving(false);
      }
    },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await removeLearningPathCourse(modal.item.id);
      await load();
      setBanner("");
      closeModal();
    } catch (err) {
      setBanner(err instanceof Error ? err.message : "Unable to remove.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  async function handleToggle(item: LearningPathCourse) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-5">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Learning Path Courses</h2>
          <p className="mt-0.5 text-sm text-slate-500">
            Assign and sequence courses within learning paths.
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-3 border-b border-slate-100 px-6 py-4">
        <div className="relative min-w-[220px] flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by course or path name…"
        ...
        {!loading && (
          <p className="self-center text-sm text-slate-400">
            {filtered.length} {filtered.length === 1 ? "entry" : "entries"}{search && " found"}
          </p>
        )}
      </div>
                    ...
                    <tr key={item.id} className="transition hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800">
                          {findName(learningPaths, item.learning_path_id, "path_name")}
                        </p>
                        <p className="mt-0.5 text-xs text-slate-400">
                          {findName(learningPaths, item.learning_path_id, "path_code")}
                        </p>
                      </td>

                      <td className="px-4 py-3 text-slate-700">
                        {findName(courses, item.course_id, "course_name")}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center text-slate-600">
                        {item.estimated_duration > 0 ? `${item.estimated_duration}m` : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                          item.unlock_previous
                            ? "bg-blue-50 text-blue-700 ring-1 ring-blue-200"
                            : "bg-slate-100 text-slate-400 ring-1 ring-slate-200"
                        }`}>
                          {item.unlock_previous ? "Yes" : "No"}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        ...
                        >
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                            ...
                            <span className={`h-1.5 w-1.5 rounded-full ${item.active ? "bg-emerald-500" : "bg-slate-400"}`} />
      ...
      {modal?.type === "delete" && (
        <DeleteDialog
          name={findName(courses, modal.item.course_id, "course_name")}
          busy={deleting}
