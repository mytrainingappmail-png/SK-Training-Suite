...
function BranchManagement() {
  ...
  // ── Search filter — useEffect is the single source of truth for filteredBranches.
  // setBranches (optimistic updates, loadData) all flow through here automatically.
  ...
  async function handleToggleStatus(branch: Branch) {
    ...
    try {
      ...
      // Optimistic local update — flows through the search useEffect automatically
      setBranches((prev) =>
        prev.map((b) =>
  ...
  // When turning ON, reload fully: backend may clear head_office on other rows.
  // When turning OFF, optimistic local update is safe — no cross-row side effects.
