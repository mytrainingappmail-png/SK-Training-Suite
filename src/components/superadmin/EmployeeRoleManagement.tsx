...
import {
  ...
  removeEmployeeRole,
  toggleActive,
} from "../../services/employeeRole/employeeRoleService";
import { employeeService } from "../../services/employee/employeeService";
import { loadRoles }       from "../../services/role/roleService";

import type { EmployeeRole, EmployeeRoleForm } from "../../types/employeeRole";
import type { Employee }                        from "../../types/employee";
import type { Role }                            from "../../types/role";
import { defaultEmployeeRoleForm }              from "../../types/employeeRole";
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="mb-1 text-base font-semibold text-slate-800">
        {search ? "No assignments found" : "No employee role assignments yet"}
      </h3>
      <p className="mb-6 text-sm text-slate-500">
        {search ? `No results for "${search}".` : "Assign a role to an employee to get started."}
      </p>
      {!search && (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="del-title">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <h3 id="del-title" className="mb-1 text-lg font-semibold text-slate-800">Remove Role Assignment</h3>
        <p className="mb-6 text-sm text-slate-500">
          Are you sure you want to remove the role assignment for <span className="font-semibold text-slate-700">{name}</span>? This cannot be undone.
        </p>
...
function EmployeeRoleModal({
  ...
  roles: Role[];
  saving: boolean;
  onSave: (data: EmployeeRoleForm) => void;
  onClose: () => void;
}) {
  ...
  function field<K extends keyof EmployeeRoleForm>(key: K, val: EmployeeRoleForm[K]) {
    setForm((p) => ({ ...p, [key]: val }));
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto py-10" role="dialog" aria-modal="true" aria-labelledby="er-form-title">
      ...
      <div className="relative z-10 w-full max-w-xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 id="er-form-title" className="text-lg font-semibold text-slate-800">
              {isEdit ? "Edit Role Assignment" : "Assign Role"}
            </h2>
            <p className="text-sm text-slate-500">
              {isEdit ? "Update role assignment details." : "Assign a role to an employee."}
            </p>
          </div>
        ...
        <form onSubmit={onSubmit} noValidate>
          <div className="space-y-5 p-6">
            ...
            <FL label="Role" required error={errs.role_id}>
            ...
            <FL label="Assigned Date" required error={errs.assigned_date}>
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <ToggleRow
                label="Active"
                sub="Role assignment is currently active"
                on={form.active}
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? "Saving…" : isEdit ? "Update Assignment" : "Assign Role"}
            </button>
          </div>
        </form>
...
type ModalKind =
  | { type: "add" }
  | { type: "edit";   item: EmployeeRole }
  | { type: "delete"; item: EmployeeRole }
  | null;

export default function EmployeeRoleManagement() {

  const [items,     setItems]     = useState<EmployeeRole[]>([]);
  ...
  const filtered = useMemo(() => {
    ...
    if (kw) {
      rows = rows.filter((r) => {
        ...
        const rn = findName(roles, r.role_id, "role_name");
        return (
          en.toLowerCase().includes(kw) ||
          rn.toLowerCase().includes(kw) ||
          r.assigned_date.includes(kw)
        );
      });
    }
    return rows;
  }, [items, employees, roles, search, empFilter]);
  ...
  useEffect(() => { setPage(1); }, [search, empFilter]);
  ...
  const load = useCallback(async () => {
    ...
    setBanner("");
    try {
      const [iData, eData, rData] = await Promise.all([
        loadEmployeeRoles(),
  ...
  const handleSave = useCallback(
    async (data: EmployeeRoleForm) => {
      setSaving(true);
      try {
        if (modal?.type === "edit") {
          await saveEmployeeRole(modal.item.id, data);
        } else {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await removeEmployeeRole(modal.item.id);
      await load();
      setBanner("");
      closeModal();
    } catch (err) {
      setBanner(err instanceof Error ? err.message : "Unable to remove assignment.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  async function handleToggleActive(item: EmployeeRole) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-5">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Employee Role Management</h2>
              ...
              <thead className="bg-slate-50">
                <tr>
                  {[
                    ...
                    { h: "Assigned Date", cls: "text-left"      },
                    { h: "Status",        cls: "text-center"    },
                    { h: "Created",       cls: "text-left"      },
                  ...
                  return (
                    <tr key={item.id} className="transition hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800">{empName(employees, item.employee_id)}</p>
                      </td>

                      <td className="px-4 py-3">
                        <p className="text-slate-700">{findName(roles, item.role_id, "role_name")}</p>
                        <p className="mt-0.5 font-mono text-xs text-slate-400">{findName(roles, item.role_id, "role_code")}</p>
                      </td>

                      <td className="px-4 py-3 text-slate-600">{item.assigned_date || "—"}</td>
      ...
      {modal?.type === "delete" && (
        <DeleteDialog
          name={`${empName(employees, modal.item.employee_id)} → ${findName(roles, modal.item.role_id, "role_name")}`}
          busy={deleting}
