...
import { defaultLearningPathForm } from "../../types/learningPath";
...
function difficultyColour(level: DifficultyLevel): string {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="mb-1 text-base font-semibold text-slate-800">
        {search ? "No learning paths found" : "No learning paths yet"}
      </h3>
      <p className="mb-6 text-sm text-slate-500">
        {search ? `No results for "${search}".` : "Create your first learning path to get started."}
      </p>
      {!search && (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-modal="true" aria-labelledby="dd-title">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <h3 id="dd-title" className="mb-1 text-lg font-semibold text-slate-800">Delete Learning Path</h3>
        <p className="mb-6 text-sm text-slate-500">
...
function LearningPathModal({
  ...
  usedCodes: string[];
  saving: boolean;
  onSave: (data: LearningPathForm) => void;
  onClose: () => void;
}) {
  const isEdit = editing !== null;

  const [form, setForm] = useState<LearningPathForm>(() =>
    isEdit
      ? {
          path_code:               editing.path_code,
          path_name:               editing.path_name,
          ...
          prerequisite_path_id:    editing.prerequisite_path_id,
  ...
  // Paths available as prerequisite — exclude self
  const prerequisitePaths = useMemo(
    () => allPaths.filter((p) => !isEdit || p.id !== editing.id),
    [allPaths, isEdit, editing]
  );
  ...
  function field<K extends keyof LearningPathForm>(key: K, val: LearningPathForm[K]) {
    setForm((p) => ({ ...p, [key]: val }));
  ...
  function validate(): boolean {
    ...
    if (!form.path_code.trim()) {
      e.path_code = "Path Code is required.";
    } else {
      const code = form.path_code.trim().toLowerCase();
      const dup = usedCodes.includes(code) &&
        (!isEdit || code !== editing.path_code.trim().toLowerCase());
      if (dup) e.path_code = "Path Code already exists.";
    }

    if (!form.path_name.trim()) {
      e.path_name = "Path Name is required.";
    }

    const allowed: DifficultyLevel[] = ["beginner", "intermediate", "advanced"];
    if (!allowed.includes(form.difficulty_level)) {
      e.difficulty_level = "Difficulty Level must be beginner, intermediate, or advanced.";
    }

    if (form.estimated_duration < 0) {
      e.estimated_duration = "Estimated Duration cannot be negative.";
    }

    if (form.display_order < 1) {
      e.display_order = "Display Order must be greater than zero.";
    }
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 id="lp-form-title" className="text-lg font-semibold text-slate-800">
              {isEdit ? "Edit Learning Path" : "Add Learning Path"}
            </h2>
            <p className="text-sm text-slate-500">
              {isEdit ? "Update learning path details." : "Define a new structured learning path."}
            </p>
          </div>
        ...
        <form onSubmit={onSubmit} noValidate>
          <div className="space-y-5 p-6">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <FL label="Path Code" required error={errs.path_code}>
                <input ref={firstRef} type="text" value={form.path_code}
                  onChange={(e) => field("path_code", e.target.value)}
                  placeholder="e.g. LP-001"
                  maxLength={30} disabled={saving} className={CLS_INPUT} />
              </FL>
              <FL label="Path Name" required error={errs.path_name}>
                <input type="text" value={form.path_name}
                  onChange={(e) => field("path_name", e.target.value)}
            ...
            <FL label="Description">
              <textarea value={form.description}
                ...
                placeholder="Optional description of this learning path"
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
              <FL label="Difficulty Level" required error={errs.difficulty_level}>
              ...
              <FL label="Estimated Duration (min)" error={errs.estimated_duration}>
                <input type="number" min={0} value={form.estimated_duration}
                  onChange={(e) =>
                    field("estimated_duration", Math.max(0, parseInt(e.target.value, 10) || 0))
                  }
                  disabled={saving} className={CLS_INPUT} />
              </FL>
            ...
            {/* ── Relations ── */}
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Relations</p>

            <FL label="Prerequisite Learning Path">
              <select
                value={form.prerequisite_path_id ?? ""}
                onChange={(e) =>
                  field("prerequisite_path_id", e.target.value || null)
                }
                disabled={saving} className={CLS_SELECT}>
                <option value="">— None —</option>
                {prerequisitePaths.map((p) => (
                  <option key={p.id} value={p.id}>{p.path_code} — {p.path_name}</option>
                ))}
              </select>
            </FL>

            <FL label="Certificate Template">
              <select
                value={form.certificate_template_id ?? ""}
                onChange={(e) =>
                  field("certificate_template_id", e.target.value || null)
                }
                disabled={saving} className={CLS_SELECT}>
                <option value="">— None —</option>
                {templates.map((t) => (
          ...
          <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4">
            ...
            <button type="submit" disabled={saving}
              ...
              {saving ? "Saving…" : isEdit ? "Update Path" : "Add Path"}
            </button>
          </div>
        </form>
...
type ModalKind =
  | { type: "add" }
  | { type: "edit"; path: LearningPath }
  | { type: "delete"; path: LearningPath }
  | null;
...
export default function LearningPathManagement() {
  ...
  const usedCodes = useMemo(
    () => paths.map((p) => p.path_code.trim().toLowerCase()),
    [paths]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return paths.filter(
      (p) =>
        p.path_code.toLowerCase().includes(kw) ||
        p.path_name.toLowerCase().includes(kw) ||
        (p.description ?? "").toLowerCase().includes(kw) ||
        p.difficulty_level.toLowerCase().includes(kw)
    );
  }, [paths, search]);
  ...
  const load = useCallback(async () => {
    ...
    setBanner("");
    try {
      const [pathData, tmplData] = await Promise.all([
        loadLearningPaths(),
        loadTemplates(),
      ]);
      setPaths(pathData);
  ...
  const handleSave = useCallback(
    async (data: LearningPathForm) => {
      setSaving(true);
      try {
        if (modal?.type === "edit") {
          await saveLearningPath(modal.path.id, data);
        } else {
        ...
        setBanner(err instanceof Error ? err.message : "Unable to save learning path.");
      } finally {
        setSaving(false);
      }
    },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await removeLearningPath(modal.path.id);
      await load();
      setBanner("");
      closeModal();
    } catch (err) {
      setBanner(err instanceof Error ? err.message : "Unable to delete learning path.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  async function handleToggleActive(path: LearningPath) {
    setTogglingActId(path.id);
    try {
      await toggleActive(path.id, !path.active);
      setPaths((prev) =>
        prev.map((p) => p.id === path.id ? { ...p, active: !path.active } : p)
      );
    } catch (err) {
  ...
  async function handleTogglePublished(path: LearningPath) {
    setTogglingPubId(path.id);
    try {
      await togglePublished(path.id, !path.published);
      setPaths((prev) =>
        prev.map((p) => p.id === path.id ? { ...p, published: !path.published } : p)
      );
    } catch (err) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-5">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Learning Path Management</h2>
      ...
      <div className="flex flex-wrap items-center gap-4 px-6 py-4">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="Search by code, name, difficulty…"
        ...
        {!loading && (
          <p className="text-sm text-slate-400">
            {filtered.length} {filtered.length === 1 ? "path" : "paths"}{search && " found"}
          </p>
        )}
      </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="min-w-full divide-y divide-slate-100 text-sm">
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {pageRows.map((path, i) => {
                  const busyAct = togglingActId === path.id;
                  const busyPub = togglingPubId === path.id;
                  const busy    = busyAct || busyPub;
                  return (
                    <tr key={path.id} className="transition hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800">{path.path_name}</p>
                        {path.description && (
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${difficultyColour(path.difficulty_level)}`}>
                          {path.difficulty_level.charAt(0).toUpperCase() + path.difficulty_level.slice(1)}
                        </span>
                      </td>

                      <td className="px-4 py-3 text-center text-slate-600">
                        {path.estimated_duration > 0 ? `${path.estimated_duration}m` : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3 text-slate-500 text-xs">
                        {findName(paths, path.prerequisite_path_id, "path_name")}
                      </td>

                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => handleTogglePublished(path)}
                          disabled={busy}
                        ...
                        >
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                            ...
                            <span className={`h-1.5 w-1.5 rounded-full ${path.published ? "bg-blue-500" : "bg-slate-400"}`} />
                      ...
                      <td className="px-4 py-3 text-center">
                        ...
                        >
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                            ...
                            <span className={`h-1.5 w-1.5 rounded-full ${path.active ? "bg-emerald-500" : "bg-slate-400"}`} />
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => openModal({ type: "edit", path })}
                            disabled={busy}
                          ...
                          <button
                            onClick={() => openModal({ type: "delete", path })}
                            disabled={busy}
      ...
      {(modal?.type === "add" || modal?.type === "edit") && (
        <LearningPathModal
          editing={modal.type === "edit" ? modal.path : null}
          allPaths={paths}
          templates={templates}
          usedCodes={usedCodes}
          saving={saving}
