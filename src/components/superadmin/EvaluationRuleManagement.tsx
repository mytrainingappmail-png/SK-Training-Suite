...
import type { EvaluationRule, EvaluationRuleForm } from "../../types/evaluation";
import type { Assessment } from "../../types/assessment";
import { defaultEvaluationRuleForm } from "../../types/evaluation";
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h3 className="mb-1 text-base font-semibold text-slate-800">
        {search ? "No rules found" : "No evaluation rules yet"}
      </h3>
      <p className="mb-6 text-sm text-slate-500">
        {search
          ? `No results for "${search}".`
          : "Add an evaluation rule to control how assessments are scored."}
      </p>
      {!search && (
        <button
          onClick={onAdd}
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <p className="mb-6 text-sm text-slate-500">
          Are you sure you want to delete the evaluation rule for{" "}
...
function EvaluationRuleModal({
  ...
  editing: EvaluationRule | null;
  assessments: Assessment[];
  takenAssessmentIds: string[];
  saving: boolean;
  onSave: (data: EvaluationRuleForm) => void;
  onClose: () => void;
}) {
  const isEdit = editing !== null;

  const [form, setForm] = useState<EvaluationRuleForm>(() =>
    isEdit
      ? {
          assessment_id:             editing.assessment_id,
          negative_marking:          editing.negative_marking,
          negative_marks:            editing.negative_marks,
          partial_marking:           editing.partial_marking,
          pass_percentage:           editing.pass_percentage,
          case_sensitive_fill_blank: editing.case_sensitive_fill_blank,
          trim_whitespace:           editing.trim_whitespace,
          allow_manual_review:       editing.allow_manual_review,
          active:                    editing.active,
        }
      : { ...defaultEvaluationRuleForm }
  );
  ...
  function field<K extends keyof EvaluationRuleForm>(
    key: K,
    val: EvaluationRuleForm[K]
  ) {
    setForm((p) => ({ ...p, [key]: val }));
  ...
  function validate(): boolean {
    ...
    if (!form.assessment_id) {
      e.assessment_id = "Assessment is required.";
    } else if (
      takenAssessmentIds.includes(form.assessment_id) &&
      (!isEdit || form.assessment_id !== editing.assessment_id)
    ) {
      e.assessment_id = "An evaluation rule already exists for this assessment.";
    }

    if (form.pass_percentage < 0 || form.pass_percentage > 100) {
      e.pass_percentage = "Pass Percentage must be between 0 and 100.";
    }

    if (form.negative_marks < 0) {
      e.negative_marks = "Negative Marks must be zero or greater.";
    }
      ...
      <div className="relative z-10 w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div>
            <h2 id="er-form-title" className="text-lg font-semibold text-slate-800">
              {isEdit ? "Edit Evaluation Rule" : "Add Evaluation Rule"}
            </h2>
            <p className="text-sm text-slate-500">
              {isEdit
                ? "Update scoring and evaluation settings."
                : "Define how this assessment will be evaluated."}
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={onSubmit} noValidate>
          <div className="space-y-5 p-6">
            ...
            <FL label="Pass Percentage (%)" required error={errs.pass_percentage}>
              <input
                type="number"
                min={0}
                max={100}
            ...
            <div className="space-y-4 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <ToggleRow
                ...
                sub="Deduct marks for incorrect answers"
              ...
              <ToggleRow
                ...
                sub="Award partial marks for partially correct answers"
                on={form.partial_marking}
                onChange={() => field("partial_marking", !form.partial_marking)}
                disabled={saving}
              />
            </div>
            ...
            <div className="flex flex-wrap gap-y-4 gap-x-8 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <ToggleRow
                label="Case-Sensitive Fill in the Blank"
                sub="Exact case required for fill-in-the-blank answers"
                on={form.case_sensitive_fill_blank}
                onChange={() =>
                  field("case_sensitive_fill_blank", !form.case_sensitive_fill_blank)
                }
                disabled={saving}
              />
              <ToggleRow
                label="Trim Whitespace"
                sub="Ignore leading and trailing spaces in text answers"
                on={form.trim_whitespace}
                onChange={() => field("trim_whitespace", !form.trim_whitespace)}
                disabled={saving}
              />
              <ToggleRow
                label="Allow Manual Review"
                sub="Enable instructor review of short/long answers"
                on={form.allow_manual_review}
                onChange={() =>
                  field("allow_manual_review", !form.allow_manual_review)
                }
                disabled={saving}
              />
              <ToggleRow
                label="Active"
                sub="Rule is applied during evaluation"
                on={form.active}
...
type ModalKind =
  | { type: "add" }
  | { type: "edit"; rule: EvaluationRule }
  | { type: "delete"; rule: EvaluationRule }
  | null;

export default function EvaluationRuleManagement() {
  ...
  // ── Derived
  const takenAssessmentIds = useMemo(
    () => rules.map((r) => r.assessment_id),
    [rules]
  );
  ...
  const filtered = useMemo(() => {
    ...
    return rules.filter((r) => {
      const name = assessments.find((a) => a.id === r.assessment_id)
        ?.assessment_title ?? "";
      return name.toLowerCase().includes(kw);
    });
  }, [rules, assessments, search]);
  ...
  const load = useCallback(async () => {
    ...
    setBanner("");
    try {
      const [ruleData, assessmentData] = await Promise.all([
        loadRules(),
        loadAssessments(),
      ]);
      setRules(ruleData);
  ...
  const handleSave = useCallback(
    async (data: EvaluationRuleForm) => {
      setSaving(true);
      try {
        if (modal?.type === "edit") {
          await saveRule(modal.rule.id, data);
        } else {
        ...
        setBanner(err instanceof Error ? err.message : "Unable to save rule.");
      } finally {
        setSaving(false);
      }
    },
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await removeRule(modal.rule.id);
      await load();
      setBanner("");
      closeModal();
    } catch (err) {
      setBanner(err instanceof Error ? err.message : "Unable to delete rule.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  // ── Toggle — optimistic
  async function handleToggle(rule: EvaluationRule) {
    setTogglingId(rule.id);
    try {
      await toggleRuleStatus(rule.id, !rule.active);
      setRules((prev) =>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-6 py-5">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Evaluation Rules</h2>
          <p className="mt-0.5 text-sm text-slate-500">
            Configure scoring and evaluation rules for each assessment.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-4 px-6 py-4">
        ...
        {!loading && (
          <p className="text-sm text-slate-400">
            {filtered.length} {filtered.length === 1 ? "rule" : "rules"}{search && " found"}
          </p>
        )}
      </div>
              ...
              <thead className="bg-slate-50">
                <tr>
                  {[
                    ...
                    { h: "Neg. Marks",      cls: "text-center"    },
                    { h: "Partial",         cls: "text-center"    },
                    { h: "Manual Review",   cls: "text-center"    },
                    ...
                    <tr key={rule.id} className="transition hover:bg-slate-50/60">
                      ...
                      <td className="px-4 py-3">
                        <p className="font-semibold text-slate-800">
                          {findName(assessments, rule.assessment_id, "assessment_title")}
                        </p>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${
                            rule.partial_marking
                              ? "bg-blue-50 text-blue-700 ring-1 ring-blue-200"
                              : "bg-slate-100 text-slate-400 ring-1 ring-slate-200"
                          }`}
                        >
                          {rule.partial_marking ? "Yes" : "No"}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${
                            rule.allow_manual_review
                              ? "bg-amber-50 text-amber-700 ring-1 ring-amber-200"
                              : "bg-slate-100 text-slate-400 ring-1 ring-slate-200"
                          }`}
                        >
                          {rule.allow_manual_review ? "Yes" : "No"}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => openModal({ type: "edit", rule })}
                            disabled={busy}
                          ...
                          <button
                            onClick={() => openModal({ type: "delete", rule })}
                            disabled={busy}
      ...
      {(modal?.type === "add" || modal?.type === "edit") && (
        <EvaluationRuleModal
          editing={modal.type === "edit" ? modal.rule : null}
          assessments={assessments}
          takenAssessmentIds={takenAssessmentIds}
          saving={saving}
          onSave={handleSave}
          onClose={closeModal}
        />
      ...
      {modal?.type === "delete" && (
        <DeleteDialog
          name={findName(assessments, modal.rule.assessment_id, "assessment_title")}
          busy={deleting}
